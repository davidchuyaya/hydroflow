...
pub fn pbft<'a, D: Deploy<'a, ClusterId = u32>>(
    ...
    client_spec: &impl ClusterSpec<'a, D>,
    replicas_spec: &impl ClusterSpec<'a, D>,
    // f: RuntimeData<&'a u32>,
) -> (D::Cluster, D::Cluster) {
    // let f = 1u32;
    // Assume single client.
    let client = flow.cluster(client_spec);
    ...
    /* Start from here */
    // // !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    // // Comment out to test if it compiles.
                    ...
                    .map(|idx| {
                        deployment.add_service(
                            HydroflowCrate::new(".", localhost.clone())
                                .bin("pbft")
                                .profile(profile)
                                .display_name(format!("client/{}", idx)),
